...
MODELNAME = os.getenv("MODELNAME","hf-hub:laion/CLIP-ViT-B-32-laion2B-s34B-b79K")
...
class Clip:
    ...
    def load(self):
        ...
        modelNode['model'], Clip.models[self.modelName]['preprocess'] = open_clip.create_model_from_pretrained(self.modelName)
        modelNode['tokenizer'] = open_clip.get_tokenizer(self.modelName)
        if modelNode.get('backgroundThread')==None:
            modelNode['background_thread']=threading.Thread(target=self.background_task)
        ...
        try:
            ...
            if devcount>0 and DEVICE!="cpu": 
                ...
                print(f"loaded on {devicename}, {freemem/1024/1024:.0f} MB left")
            else:
                device="cpu"
        ...
        while True:
            ...
            if None!=modelNode and None!=modelNode.get('device') and time.time()-modelNode['lastAccessed']>int(TIMEOUT):
                ...
                if device!='cpu':
                    freemem = torch.cuda.mem_get_info()[0]
                    devicename=torch.cuda.get_device_name(device)
                    print(f"unloaded from {devicename}, {freemem/1024/1024:.0f} MB left")
...
def embed():
    ...
    ret={"embeddings": embeddings.cpu().numpy().tolist()}   
    return ret
